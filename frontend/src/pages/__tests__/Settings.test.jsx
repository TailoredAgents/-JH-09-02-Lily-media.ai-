...
describe('Settings', () => {
  ...
  it('switches between tabs correctly', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('Intent Confidence Threshold')
      ).toBeInTheDocument()
    })
  })
  ...
  it('displays and allows editing booking settings', async () => {
    ...
    const requirePhotosToggle = screen
      .getByText('Require Photos')
      .closest('div')
  ...
  it('saves pricing settings when save button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('pricing settings saved successfully!')
      ).toBeInTheDocument()
    })
  })
  ...
  it('saves weather settings when save button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('weather settings saved successfully!')
      ).toBeInTheDocument()
    })
  })
  ...
  it('saves booking settings when save button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('booking settings saved successfully!')
      ).toBeInTheDocument()
    })
  })

  it('displays error messages when API calls fail', async () => {
    api.updatePricingSettings = jest
      .fn()
  ...
  it('shows loading state initially', () => {
    ...
    expect(
      screen.getByRole('generic', { className: /animate-spin/ })
    ).toBeInTheDocument()
  })

  it('handles settings load failure gracefully', async () => {
    api.getPricingSettings = jest
      .fn()
      .mockRejectedValue(new Error('Load failed'))
    api.getWeatherSettings = jest
      .fn()
      .mockRejectedValue(new Error('Load failed'))
    api.getBookingSettings = jest
      .fn()
      .mockRejectedValue(new Error('Load failed'))
  ...
  it('shows plan-gated features correctly', async () => {
    ...
    const seasonalSection = screen
      .getByText('Seasonal Modifiers')
      .closest('div')
  ...
  it('toggles auto-reschedule settings correctly', async () => {
    ...
    const autoRescheduleToggle = screen
      .getByText('Enable Auto-Reschedule')
      .closest('div')
      .querySelector('button')
    ...
    await waitFor(() => {
      expect(
        screen.queryByText('Notification Hours Ahead')
  ...
  it('toggles quiet hours correctly', async () => {
    ...
    const quietHoursToggle = screen
      .getByText('Enable Quiet Hours')
      .closest('div')
      .querySelector('button')
